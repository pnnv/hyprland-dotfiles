<script lang="ts">
  ...
  onMount(async () => {
    try {
      const response = await fetch('/cp.json'); // Use './cp.json' if it's in the same directory
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      let data: cp[] = await response.json();

      // Example processing: Ensure links do not end with a slash
      pps = data.map((item) => {
