<script lang="ts">
	...
	import type { cp } from '../../util/types';
	...
	let pps: cp[];
	onMount(async () => {
		const response = await fetch('cp.json');
		let unpatched = await response.json();
		// patch repo owners having a slash at the end of them
		for (let i = 0; i < unpatched.length; i++) {
			const element = unpatched[i];
...
<section class="wrapper" id="profiles">
  <div class="title">
    <h2><span>Coding</span> Profiles</h2>
  </div>
  <div class="grid">
    {#if loading}
      ...
      {/each}
    {:else}
      {#each pps as { platform, link, username }}
        <a href={link} target="_blank" rel="noreferrer" class="profile-card">
          <div class="info">
            <img src="https://www.google.com/s2/favicons?sz=64&domain={link}" alt="{platform} logo" />
            <h3>{platform}</h3>
