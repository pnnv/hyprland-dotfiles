...
vector<long double> dijkstra(int start, const vector<vector<pair<int, long double>>>& g, int n) {
    vector<long double> dist(n + 1, INT_MAX);
    dist[start] = 0;
...
int32_t main() {
  ios::sync_with_stdio(false); cin.tie(0);
	...
	for(auto &i: w) cin >> i;
	for(int i = 0; i < m; i++) {
		int a, b; long double c; cin >> a >> b >> c; --a; --b;
