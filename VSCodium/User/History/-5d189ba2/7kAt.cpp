...
//   public:
    vector<int> treeQueries(int N, vector<int> &A, vector<vector<int>> &G, int Q, vector<vector<int>> &queries) {
		...
		vector <int> bgin(N), ed(N), order;
		int time = 0;

		function <int(int, int)> dfs = [&](int p, int c) {
			bgin[c] = time++; order.push_back(c);
			for(auto v: t[c])
				if(v != p) dfs(c, v);
			return ed[c] = time;
		}; dfs(-1, 0);
		for(auto i: order) a.push_back(mi(A[i]));
		...
		for(auto i: queries) {
			if(i[0] == 1) {
				...
				fin.push_back(int(st.prod(i[1], i[1] + ed[i[1]] - bgin[i[1]])));
		}
		for(auto i: a) cout << int(i) << ' '; cout << endl;
