...
void snu() {
	...
	if(ns.empty() && cout << s << ' ' << s.front() << endl) return;
	...
	
	{
		...
		string T = ini, t = "";
		for(auto c: rem) {
			for(auto i: T) if(i != c) t.push_back(i);
			if(t != s.substr(i, t.size()) && cout << "-1\n") return;
			i += t.size();
			T = t; t = "";
		}
	}
