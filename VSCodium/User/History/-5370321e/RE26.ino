...
void loop() {
  ...
  if (myFile) {
    ...
    Serial.print(
        F("SD Card: Issue encountered while attempting to open the file "));
...
void sendPacket(const char *data) {
  ...
  while (token != NULL) {
    ...
    uint8_t checksum =
        0xFF - ((frameType + frameID + destAddr[0] + destAddr[1] + destAddr[2] +
                 destAddr[3] + destAddr[4] + destAddr[5] + destAddr[6] +
                 destAddr[7] + destAddr16[0] + destAddr16[1] + options +
                 radius + sum(dataBytes, dataLength)) &
                0xFF);
