...
void sendPacket(const char *data) {
  ...
  byte destAddr[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF}; // Broadcast address
  ...
  xbeeSerial.write(dataLength + 14); // Length of the entire packet
  xbeeSerial.write(frameType);
  xbeeSerial.write(frameID);
  xbeeSerial.write(destAddr, 8);
  xbeeSerial.write(destAddr16, 2);
  xbeeSerial.write(options);
  xbeeSerial.write(radius);
  xbeeSerial.write(data, dataLength);
